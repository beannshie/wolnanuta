...
<ul class="dropdown dropdown-horizontal">
    ...
    <li class="node"><a href="#">Menu</a>
        <ul>
            <?php include_partial('menu_list', array('items' => $items, 'items_in_menu' => true)); ?>
        </ul>
    </li>
    <?php  endif; ?>
    <?php include_partial('menu_list', array('items' => $items, 'items_in_menu' => false)); ?>
</ul>
<?php endif; ?>
...
<ul class="dropdown dropdown-horizontal">
    ...
    <?php   if (sfAdminDash::hasPermission($category, $sf_user)): ?>
        <?php     if (sfAdminDash::hasItemsMenu($category['items'])): ?>
            <li class="node"><a href="#"><?php echo isset($category['name']) ? $category['name'] : $name ?></a>
                <ul>
                    <?php include_partial('menu_list', array('items' => $category['items'], 'items_in_menu' => true)) ?>
                </ul>
            </li>
            <?php     endif; ?>
        <?php   endif; ?>
    <?php endforeach; ?>
    <?php foreach ($categories as $name => $category): ?>
    <?php include_partial('menu_list', array('items' => $category['items'], 'items_in_menu' => false)) ?>
    <?php endforeach; ?>
</ul>
