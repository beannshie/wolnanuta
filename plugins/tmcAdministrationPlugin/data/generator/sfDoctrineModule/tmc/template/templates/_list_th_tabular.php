...
<?php if ($field->isReal()): ?>
  [?php if ('<?php echo $name ?>' == $sort[0]): ?]
    [?php echo image_tag(sfConfig::get('sf_admin_module_web_dir').'/images/'.$sort[1].'.png', array('alt' => __($sort[1], array(), 'tmcAdministrationPlugin'), 'title' => __($sort[1], array(), 'tmcAdministrationPlugin'))) ?]
